...
import static com.example.bletestapp.bluetoothLe.GattAttributes.UUID_CHARACTERISTIC_BATTERY_LEVEL;
...
public class BluetoothLeService extends Service {
	...
	private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
		...
		public void onCharacteristicRead(BluetoothGatt gatt,
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				if (GattAttributes.UUID_CHARACTERISTIC_HEART_RATE_BODY_PART.equals(characteristic.getUuid())) {
					decodeBodySensorPosition(characteristic);
				} else if(UUID_CHARACTERISTIC_BATTERY_LEVEL.equals(characteristic.getUuid())){
					decodeBatteryLevel(characteristic);
				}
		...
		public void onCharacteristicChanged(BluetoothGatt gatt,
			...
			if (GattAttributes.UUID_CHARACTERISTIC_CYCLING_SPEED_AND_CADENCE_MEASUREMENT.equals(charaUUID)) {
				decodeSpeedCharacteristic(characteristic);
			} else if (GattAttributes.UUID_CHARACTERISTIC_HEART_RATE_MEASUREMENT.equals(charaUUID)) {
				decodeHeartRateCharacteristic(characteristic);
				...
				decodeBPMICPCharacteristic(characteristic);
			} else if (GattAttributes.UUID_CHAR_TEMPERATURE_UUID.equals(charaUUID)) {
				decodeTemperatureCharacteristic(characteristic);
			} else if (GattAttributes.UUID_CHARACTERISTIC_CYCLING_POWER_MEASUREMENT.equals(charaUUID)) {
				//decodeCyclingPowerCharacteristic(characteristic);
			} else if (GattAttributes.UUID_CHARACTERISTIC_RUNNING_SPEED_AND_CADENCE_MEASUREMENT.equals(charaUUID)) {
				decodeRunningSpeedCharacteristic(characteristic);
	...
	private void decodeRunningSpeedCharacteristic(BluetoothGattCharacteristic characteristic) {
	...
	private void decodeBatteryLevel(BluetoothGattCharacteristic characteristic){
	...
	private void decodeTemperatureCharacteristic(BluetoothGattCharacteristic characteristic) {
	...
	private void decodeBPMICPCharacteristic(BluetoothGattCharacteristic characteristic) {
	...
	private void decodeHeartRateCharacteristic(BluetoothGattCharacteristic characteristic) {
		int flag = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, 0);
	...
	private void decodeBodySensorPosition(BluetoothGattCharacteristic characteristic) {
	...
	private void decodeSpeedCharacteristic(BluetoothGattCharacteristic characteristic) {
		int flag = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, 0);
